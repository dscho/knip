...
public final class DataTools {
        ...
        public static short bytesToShort(short[] bytes, int off, int len,
                ...
                short total = 0;
                for (int i = 0, ndx = off; i < len; i++, ndx++) {
                        total |= ((int) bytes[ndx]) << ((little ? i : len - i
                                        - 1) * 8);
                }
                return total;
        }
         ...
         */
        public static void fillDataArray(Object alloc, byte[] b, int bpp,
                        boolean fp, boolean little) {

                if (alloc instanceof byte[]) {
                        System.arraycopy(b, 0, (byte[]) alloc, 0, b.length);
                        return;
                }
