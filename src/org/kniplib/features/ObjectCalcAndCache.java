...
public class ObjectCalcAndCache {
        ...
        public Img<BitType> binaryMask2D(final IterableInterval<BitType> ii) {
                if (bm2dIterableInterval != ii) {
                        ...
                        binaryMask2D = new SubImg<BitType>(mask,
                                        new FinalInterval(dims), false);
                }
                return binaryMask2D;
        }
        ...
        private SecondOrderStatistics<? extends RealType> soStatistics;
        private IterableInterval<? extends RealType<?>> sosIterableInterval;

        public <T extends RealType<T>> SecondOrderStatistics<T> secondOrderStatistics(
                        final IterableInterval<T> ii, final int nrGrayLevels,
                        final int distance, MatrixOrientation orientation) {

                if (sosIterableInterval != ii) {
                        sosIterableInterval = ii;
                        soStatistics = new SecondOrderStatistics<T>(ii,
                                        nrGrayLevels, distance, orientation);
                }
                return (SecondOrderStatistics<T>) soStatistics;
