 ...
 */
public class DistanceMap<T extends RealType<T>, K extends RandomAccessibleInterval<T> & Metadata, M extends RandomAccessibleInterval<FloatType>>
                implements UnaryOperation<K, M> {
                        ...
                        for (int i = 0; i < src.numDimensions(); ++i) {
                                ...
                                if (a < MAX_DIMS) {
                                        ...
                                        throw new IllegalArgumentException(
                                                        "Too many dimensions are selected."); // TODO
                                }
                                // }
                        }
                        if (a < MIN_DIMS) {
                                throw new IllegalArgumentException(
                                                "Two dimensions have to be selected."); // TODO
                        }
                ...
                while (--pos[dim_sel[3]] >= 0) {
                        ...
                        while (--pos[dim_sel[2]] >= 0) {
                                ...
                                while (--pos[dim_sel[0]] >= 0) {
                                        ...
                                        g[pos[dim_sel[0]]][0] = (srcAccess
                                                        .get().getRealDouble() == 1.0 ? dim_val[0]
                                                        + dim_val[1]
                                                        : 0);
                                        for (pos[dim_sel[1]] = 1; pos[dim_sel[1]] < dim_val[1]; ++pos[dim_sel[1]]) {
                                                // Scan 1
                                                srcAccess.setPosition(pos);
                                                g[pos[dim_sel[0]]][pos[dim_sel[1]]] = (srcAccess
                                                                .get()
                                                                .getRealDouble() == 1.0) ? 1 + g[pos[dim_sel[0]]][pos[dim_sel[1]] - 1]
                                                                : 0;

                                        }
                                        pos[dim_sel[1]] = dim_val[1] - 1;
                                        while (--pos[dim_sel[1]] >= 0) { // Scan
                                                                         // 2
                                ...
                                // Second Phase
                                {
                                        ...
                                        while (--pos[dim_sel[1]] >= 0) {
                                                ...
                                                s[0] = 0;
                                                t[0] = 0;
                                                for (int u = 1; u < dim_val[0]; ++u) { // Scan
                                                                                       // 3
                                                        while (q >= 0) {
                                                ...
                                                for (int u = dim_val[0] - 1; 0 <= u; --u) { // Scan
                                                                                            // 4
                                                        final int sq = s[q];
                                                        final int gsqy = g[s[q]][pos[dim_sel[1]]];
                                                        pos[dim_sel[0]] = u;
                        ...
                        if (dim_val[2] > 1) {
                                ...
                                pos[dim_sel[0]] = dim_val[0];
                                while (--pos[dim_sel[0]] >= 0) {
                                        resAccess.setPosition(pos[dim_sel[0]],
                                                        dim_sel[0]);
                                        pos[dim_sel[1]] = dim_val[1];
                                        while (--pos[dim_sel[1]] >= 0) {
                                                ...
                                                while (--pos[dim_sel[2]] >= 0) {
                                                        ...
                                                        s[0] = 0;
                                                        t[0] = 0;
                                                        for (int u = 1; u < dim_val[2]; ++u) { // Scan
                                                                                               // 3
                                                                while (q >= 0) {
                                                                        final int tqsq = (t[q] - s[q]);
                                                                        resAccess.setPosition(
                                                        ...
                                                        for (int u = dim_val[2] - 1; 0 <= u; --u) { // Scan
                                                                // 4
                                                                resAccess.setPosition(
                                                                                u,
                                                                                dim_sel[2]);
                                                                ...
                                                                resAccess.setPosition(
                                                                                s[q],
                                                                                dim_sel[2]);
                                                                final double gsq = resAccess
                                                                                .get()
                                                                                .getRealDouble();
                                                                val_z[u] = Math.sqrt(((u - sq)
                                                                                * (u - sq) + gsq
                                                                                * gsq));
                                                                if (u == t[q]) {
                                                                        --q;
