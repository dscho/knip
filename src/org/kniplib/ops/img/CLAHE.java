...
public class CLAHE<T extends RealType<T>, K extends RandomAccessibleInterval<T> & IterableInterval<T>>
        ...
        private int histNrBins = 256;

        /** Relative histogram clip limit. */
        // try 1.01f - won't work: check upperBin in clipHistogram
        private float clipLimit = 3.0f;

        /**
         * If enabled, CLAHE is applied (cumulative histogram slope is limited).
         * If disabled only adaptive histogram equalization is applied.
         */
         ...
         *                If enabled, CLAHE is applied (cumulative histogram
         *                slope is limited). If disabled only adaptive histogram
        ...
        public K compute(K in, K r) {
                ...
                // TODO: This is unnessacary! speed it up removing this. Is here
                // because
                // of the conversion from manipulation to operation
                new IterableIntervalCopy<T>().compute(in, r);
                ...
                if (r.numDimensions() > 2) {
                        throw new IllegalArgumentException(
                                        "CLAHE only supports 2-dim images.");
                }
                ...
                if (m_ctxNrX < 2 || m_ctxNrY < 2)
                        throw new IllegalArgumentException(
                                        "A minimum of two context regions "
                                                        + "per dimension is required.");
                if (m_imgNrGrayVals < histNrBins)
                        throw new IllegalArgumentException(
                                        "Number of input gray values must be greater than "
                                                        + "number of histogram bins.");
                if (ctxXSize * ctxYSize / histNrBins < 1)
                        throw new IllegalArgumentException(
                                        "Number of histogram bins is larger than number of pixels "
                                                        + "in a contextual region.");
                if (clipLimit <= 1.5f)
                        throw new IllegalArgumentException(
                                        "Clip limit factor must be greater 0.");
                if (absClipLimit * histNrBins < ctxNrPixels)
                        throw new IllegalArgumentException(
                                        "Relative clip limit too low.");
        ...
        /**
         * Updates the image and applies a bi-linear interpolation scheme to
         * create smooth transitions between context areas.
         * 
        ...
        private void interpolate() {
                ...
                for (int iy = 0; iy <= m_ctxNrY; iy++) {
                        ...
                        for (int ix = 0; ix <= m_ctxNrX; ix++) {
                                ...
                                int leftUpper = histNrBins
                                                * (yUp * m_ctxNrX + xLeft);
                                int rightUpper = histNrBins
                                                * (yUp * m_ctxNrX + xRight);
                                int leftBottom = histNrBins
                                                * (yBo * m_ctxNrX + xLeft);
                                int rightBottom = histNrBins
                                                * (yBo * m_ctxNrX + xRight);
                                interpolate4regions(leftUpper, rightUpper,
                                                leftBottom, rightBottom,
                                                subXSize, subYSize, imgPos);
        ...
        private void interpolate4regions(final int leftUpper,
                        final int rightUpper, final int leftBottom,
                        final int rightBottom, final int subXSize,
                        final int subYSize, int p) {
                ...
                for (int coefY = 0; coefY < subYSize; coefY++) {
                        ...
                        for (int coefX = 0; coefX < subXSize; coefX++) {
                                ...
                                // c.bck(X);
                                // }
                                m_ra.get().getRealFloat();
                                int gray = valueToBin(m_ra.get().getRealFloat());

                                int newValue = ((invCoefY
                                                * (invCoefX
                                                                * histograms[leftUpper
                                                                                + gray] + coefX
                                                                * histograms[rightUpper
                                                                                + gray]) + coefY
                                                * (invCoefX
                                                                * histograms[leftBottom
                                                                                + gray] + coefX
                                                                * histograms[rightBottom
                                                                                + gray])) / num);

                                m_ra.get().setReal(newValue);
                                p++;
                                invCoefX--;
                        }
        ...
        /**
         * Creates the histograms for all contextual regions. The image is
         * scanned line by line as this (hopefully) speeds up the underlying
         * image cursor.
         * 
         * @throws InvalidCLAHEoperation
         */
                        ...
                        for (int y = 0; y < ctxYSize; y++) {
                                for (int ctxX = 0; ctxX < m_ctxNrX; ctxX++) {
                                        for (int x = 0; x < ctxXSize; x++) {
                                                ra.fwd(0);
                                                final int bin = valueToBin(ra
                                                                .get()
                                                                .getRealDouble());
                                                final int offSet = ctxY
                                                                * m_ctxNrX
                                                                * histNrBins
                                                                + ctxX
                                                                * histNrBins;
        ...
        private void createCumulativeHistogram(final int histArrayOffset) {
                final float fScale = ((float) m_imgGrayMax - m_imgGrayMin)
                                / ctxNrPixels;
                int sum = 0;
        ...
        private void clipHistogram(final int histArrayOffset) {
                ...
                while (totalExcessPxs > 0) {
                        ...
                        while (totalExcessPxs > 0 && currHistPos < histLimit) {
                                ...
                                if (stepSize < 1)
                                        stepSize = 1;
                                for (int currBin = currHistPos; currBin < histLimit
                                                && totalExcessPxs > 0; currBin += stepSize) {
