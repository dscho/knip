...
public class VoronoiLikeRegionGrowing<L extends Comparable<L>, T extends Type<T> & Comparable<T>>
                extends
                AbstractRegionGrowing<LabelingType<L>, L, Labeling<L>, Labeling<L>> {
        ...
        /**
         * Due to the nature of the segmentation algorithm, there may be
         * background that lies completely inside cells. This background should
         * be removed (e.g. cells extended to include that background).
         */
        private void reLabelBackgroundInsideCells(Labeling<L> resLab) {

                Img<BitType> visited = null;
                try {
                        visited = new ArrayImgFactory<BitType>().imgFactory(
                                        new BitType()).create(resLab,
                                        new BitType());
                ...
                while (labCur.hasNext()) {
                        ...
                        if (!visitedCur.get().get()
                                        && labCur.get().getLabeling().isEmpty()) {
         ...
         */
        private void bfsBackGround(Labeling<L> resLab, int[] startPos,
                        RandomAccess<BitType> visitedRA) {
                ...
                List<L> labelNeighbour = resLab.firstElement().getMapping()
                                .emptyList();
                boolean ok = true;
                ...
                while (!queue.isEmpty()) {
                        ...
                        while (i > -1) {
                                ...
                                if (add) {
                                        ...
                                        if (!visitedRA.get().get()
                                                        && resLabRA.get()
                                                                        .getLabeling()
                                                                        .isEmpty()) {
                                                visitedRA.get().set(true);
                                                queue.add(nextPos.clone());
                                        } else if (!visitedRA.get().get()
                                                        && !resLabRA.get()
                                                                        .getLabeling()
                                                                        .isEmpty()) {
                                                if (labelNeighbour.isEmpty()) {
                                                        labelNeighbour = resLabRA
                                                                        .get()
                                                                        .getLabeling();
                                                } else if (labelNeighbour != resLabRA
                                                                .get()
                                                                .getLabeling()) {
                                                        ok = false;
                                                }
        ...
        public UnaryOperation<Labeling<L>, Labeling<L>> copy() {
                return new VoronoiLikeRegionGrowing<L, T>(m_srcImg,
                                m_threshold.copy(), m_fillHoles);
        }
