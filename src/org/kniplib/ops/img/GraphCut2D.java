...
public class GraphCut2D<T extends RealType<T>, I extends RandomAccessibleInterval<T> & IterableInterval<T>, O extends RandomAccessibleInterval<BitType> & IterableInterval<BitType>>
        ...
        public O compute(I src, O res) {
                ...
                for (int i = 0; i < numFeat; i++) {
                        ...
                        if (m_dimFeat != -1) {
                                ...
                                bins = hist.compute(src,
                                                hist.createEmptyOutput(src))
                                                .hist();
                        }
                ...
                final org.kniplib.algorithm.GraphCutAlgorithm graphCut = new org.kniplib.algorithm.GraphCutAlgorithm(
                ...
                while (resCursor.hasNext()) {
                        ...
                        // r_Source = (float) -Math.log(1.0 / (r_Source /
                        // (float) numFeat));
                        // r_Sink = (float) -Math.log(1.0 / (r_Sink / (float)
                        // numFeat));
                        //
                        // r_Source *= m_pottsWeight;
                        // r_Sink *= m_pottsWeight;
                        // TODO: swapped sink and source weights! -> it works,
                        // but why??
                ...
                while (resCursor.hasNext()) {
                        ...
                        resCursor.get()
                                        .set(graphCut.getTerminal(
                                                        listPosition(resPos,
                                                                        dims))
                                                        .equals(org.kniplib.algorithm.GraphCutAlgorithm.Terminal.BACKGROUND));
        ...
        public Img<BitType> createEmptyOutput(Img<T> src, long[] dims) {

                if (m_dimFeat != -1) {

                        // check dimensionality
                        if (m_sinkVal.length != src.dimension(m_dimFeat)
                                        || m_srcVal.length != src
                                                        .dimension(m_dimFeat)) {
        ...
        public UnaryOperation<I, O> copy() {
                return new GraphCut2D<T, I, O>(m_pottsWeight, m_dimX, m_dimY,
                                m_dimFeat, m_srcVal.clone(), m_sinkVal.clone());
        }
}
