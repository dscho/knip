...
public class ImgUnaryOutputOpWrapper<T extends RealType<T>> implements
        ...
        public <C> UnaryOutputOperation<C, Img<T>> concatenate(
                        final UnaryOutputOperation<C, Img<T>> op) {
                return new ConcatenatedUnaryOutputOperation<C, Img<T>, Img<T>>(
                                op, this);
        }

        public <C> UnaryOutputOperation<Img<T>, C> preConcatenate(
                        final UnaryOutputOperation<Img<T>, C> op) {
                return new ConcatenatedUnaryOutputOperation<Img<T>, Img<T>, C>(
                                this, op);
        }
}

class ConcatenatedUnaryOutputOperation<I, B, O> implements
                UnaryOutputOperation<I, O> {

        private final UnaryOutputOperation<I, B> m_first;
        private final UnaryOutputOperation<B, O> m_rest;

        public ConcatenatedUnaryOutputOperation(
                        UnaryOutputOperation<I, B> first,
                        UnaryOutputOperation<B, O> rest) {
                super();
                m_first = first;
                m_rest = rest;
        }
        ...
        public UnaryOutputOperation<I, O> copy() {
                return new ConcatenatedUnaryOutputOperation<I, B, O>(
                                m_first.copy(), m_rest.copy());
        }
