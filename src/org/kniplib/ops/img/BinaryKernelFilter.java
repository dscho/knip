...
public final class BinaryKernelFilter<T extends RealType<T>, K extends RandomAccessibleInterval<T> & IterableInterval<T>>
         ...
         * <code>m_dimIndex0</code> axis. Only the incoming pixels need to be
         * read from disk, old pixels are shifted.
         */
        ...
        public BinaryKernelFilter(final Img<BitType> kernel,
                        final int dimIndex0,
                        final UnaryOutputOperation<Iterable<T>, T> op) {
                m_dimIndex0 = dimIndex0;
                setupKernel(kernel);
                m_op = op;
        }

        public BinaryKernelFilter(Queue<T>[] kernel, final int dimIndex0,
                        final UnaryOutputOperation<Iterable<T>, T> op) {
                m_dimIndex0 = dimIndex0;
                m_kernel = kernel;
                m_op = op;
                ...
                for (int i = 0; i < kernelRadius.length; i++) {
                        if (kernelRadius[i] % 2 == 0) {
                                throw new IllegalArgumentException(
                                                "Only odd kernel sizes supported (dim["
                                                                + i
                                                                + "] = "
                                                                + kernelRadius[i]
                                                                + ").");
        ...
        public final K compute(final K op, final K r) {
                ...
                RandomAccess<T> co = Views.extendValue(op,
                ...
                while (ii.hasNext()) {
                        ...
                        for (x0 = 0; x0 < dim0; x0++) {
                                ...
                                cr.get()
                                                .set(m_op.compute(
                                                                m_kernelReferenceArray,
                                                                m_op.createEmptyOutput(m_kernelReferenceArray)));
                        }
                }
