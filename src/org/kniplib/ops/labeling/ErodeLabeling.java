...
import org.kniplib.data.StructuringElementCursor;
...
public class ErodeLabeling<L extends Comparable<L>> implements
         ...
         *                eroded individually. If the label is not present in
         *                one of the neighbor pixel it is also removed from the
         *                center.</li>
         *                <li>Binary-based: The labeling is treated as a binary
         *                image. If one of the neighbor pixels is empty the
         *                center pixel is also set to the empty list.</li>
         *                </ul>
         */
        public ErodeLabeling(final long[][] structuringElement,
        ...
        private Labeling<L> computeLabelBased(final Labeling<L> input,
                ...
                for (final L label : input.getLabels()) {
                        final Cursor<LabelingType<L>> out = input
                                        ...
                                        .getIterableIntervalOverROI(output)
                                        .localizingCursor();
                        next: while (out.hasNext()) {
                                ...
                                while (inStructure.hasNext()) {
                                        inStructure.next();
                                        if (!inStructure.get().getLabeling()
                                                        .contains(label)) {
