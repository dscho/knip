 ...
 */
public abstract class AbstractGrayscaleReconstruction<T extends Type<T>, V extends Type<V>, MASK extends RandomAccessibleInterval<T>, MARKER extends RandomAccessibleInterval<V>>
                implements UnaryOperation<MASK, MARKER> {

        /**
         * A Cursor like class, but does not confirm to Cursor from imglib.
         *
         * This is needed because a Cursor from imglib2 does not allow to
         * iterate backwards.
         */
        ...
        protected AbstractGrayscaleReconstruction(
                        final AbstractGrayscaleReconstruction<T, V, MASK, MARKER> copy) {
        ...
        public MARKER compute(MASK input, MARKER output) {
                ...
                OutOfBounds<V> marker = new OutOfBoundsConstantValueFactory<V, RandomAccessibleInterval<V>>(
                                zeroV).create(output);
                ...
                OutOfBounds<T> mask = new OutOfBoundsConstantValueFactory<T, RandomAccessibleInterval<T>>(
                                zeroT).create(input);
        ...
        private void scanInAntiRasterOrder(Cursor<V> cur,
                        RandomAccess<V> marker, RandomAccess<T> mask) {
                assert cur != null;
                assert marker != null;
                assert mask != null;
                ...
                while (cur.hasNextBwd()) {
                        ...
                        cur.get().set(checkAroundCursor(cur, marker, mask,
                                        m_neighboursMinus));
        ...
        private void scanInRasterOrder(Cursor<V> cur, RandomAccess<V> marker,
                        RandomAccess<T> mask) {
                assert cur != null;
                assert marker != null;
                assert mask != null;
        ...
        private V checkAroundCursor(Cursor<V> cur, RandomAccess<V> marker,
                        RandomAccess<T> mask, long[][] strucElement) {
                V val = cur.get().copy();
        ...
        private void checkPixels(Cursor<V> cur, RandomAccess<V> marker,
                        RandomAccess<T> mask, long[][] strucElement) {
                V p = cur.get().copy();
                ...
                for (long[] e : strucElement) {
                        ...
                        for (int i = 0; i < e.length; i++) {
                                marker.setPosition(
                                                cur.getIntPosition(i) + e[i], i);
                                mask.setPosition(cur.getIntPosition(i) + e[i],
                                                i);
                        }
        ...
        protected void propagate(RandomAccess<V> marker, RandomAccess<T> mask,
                        long[][] strucElement) {
        ...
        /*
         * These functions implement the kind of algorithm (by erosion /
         * dilation)
         */
        protected abstract boolean checkPixelFromQueue(final V p, final V q,
                        final T i);

        protected abstract boolean checkPixelAddToQueue(final V p, final V q,
                        final T i);
