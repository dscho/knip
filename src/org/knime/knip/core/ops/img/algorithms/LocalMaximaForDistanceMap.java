...
public class LocalMaximaForDistanceMap<T extends RealType<T>, K extends RandomAccessibleInterval<T> & IterableInterval<T>>
        ...
        public List<long[]> compute(final K src, final List<long[]> res) {
                ...
                } else if (src.numDimensions() > 3) {
                        throw new IllegalArgumentException(
                                        "Only three dimensions are allowed");
                }
                if (m_roi == null
                ...
                while (srcCursor.hasNext()) {
                        ...
                        if (p > 0) {
                                ...
                                if (add) {
                                        ...
                                        localMax.add(new LocalMaxima(srcCursor
                                                        .get().getRealFloat(),
                                                        pos));
                                }
                        }
                ...
                int l = localMax.size();
                for (int i = 0; i < l; ++i) {
                        final float r = localMax.get(i).val
                                        * localMax.get(i).val;
                        if (r > 1) {
                                for (int u = i + 1; u < l; ++u) {
                                        float q = 0;
                                        for (int a = 0; a < numDims; ++a) {
                                                final float d = localMax.get(u).pos[a]
                                                                - localMax.get(i).pos[a];
                                                q += d * d;
                                        }
                                        if (q < r) {
                                                localMax.remove(u);
