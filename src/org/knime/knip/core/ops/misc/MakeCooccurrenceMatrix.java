 ...
 * @param <T>
 */
public class MakeCooccurrenceMatrix<T extends RealType<T>> implements
                UnaryOperation<IterableInterval<T>, CooccurrenceMatrix> {
         ...
         */
        public MakeCooccurrenceMatrix(int dimX, int dimY, final int distance,
                        final int nrGrayLevels,
                        final CooccurrenceMatrix.MatrixOrientation orientation,
                        final BitSet enabledFeatures) {
                m_distance = distance;
        ...
        @Override
        public CooccurrenceMatrix compute(final IterableInterval<T> input,
                        final CooccurrenceMatrix output) {
                if (input == null) {
                        throw new IllegalArgumentException(
                                        "IterableInterval must not be null.");
                }
                if (output == null) {
                        throw new IllegalArgumentException(
                                        "CooccurrenceMatrix must not be null.");
                }
                ...
                m_pixels = new int[(int) input.dimension(m_dimY)][(int) input
                                .dimension(m_dimX)];
                ...
                while (cursor.hasNext()) {
                        cursor.fwd();
                        m_pixels[cursor.getIntPosition(m_dimY)
                                        - (int) input.min(m_dimY)][cursor
                                        .getIntPosition(m_dimX)
                                        - (int) input.min(m_dimX)] = (int) (((cursor
        ...
        private void computeFeatures(final CooccurrenceMatrix matrix) {
                ...
                if (m_enabledFeatures.get(2) || m_enabledFeatures.get(3)
                                || m_enabledFeatures.get(11)
                                || m_enabledFeatures.get(12)) {
                ...
                if (m_enabledFeatures.get(1) || m_enabledFeatures.get(9)
                                || m_enabledFeatures.get(10)) {
                ...
                h[4] = 0.0f; // ID(F)M
                h[8] = 0.0f; // Entropy
                if (m_enabledFeatures.get(0) || m_enabledFeatures.get(2)
                                || m_enabledFeatures.get(3)
                                || m_enabledFeatures.get(4)
                                || m_enabledFeatures.get(8)) {
                        for (int i = 0; i < m_nrGrayLevels; i++) {
                                // 2*sum_i=1^N*sum_j=i^N instead of i=1 to N and
                                // j=1 to N
                                for (int j = 0; j < m_nrGrayLevels; j++) {
                                        if (m_enabledFeatures.get(0)) {
                                                h[0] += matrix.getValueAt(i, j)
                                                                * matrix.getValueAt(
                                                                                i,
                                                                                j);
                                        }
                ...
                if (m_enabledFeatures.get(5) || m_enabledFeatures.get(6)
                        ...
                        for (int i = 2; i <= 2 * m_nrGrayLevels; i++) {
                                ...
                                if (m_enabledFeatures.get(5)
                                                && !m_enabledFeatures.get(6)) {
                                        h[5] += i * pxplusy[i];
                                }
                                // h5 <- h6
                ...
                if (m_enabledFeatures.get(13)) {
                        for (int j = 0; j < m_nrGrayLevels; j++) {
                                h[13] += Math.pow(2 * j - 2 * stdx, 3)
                                                * matrix.getValueAt(j, j);
        ...
        /**
         * Builds the co-occurrence matrix to the given image. Algorithm is
         * based on the tutorial, which means a symmetric matrix is created by
         * counting neighbor in positive and negative direction.
         *
         * @param matrix
                ...
                for (int y = 0; y < m_pixels.length; y++) {
                        for (int x = 0; x < m_pixels[y].length; x++) {
                                ...
                                if (sx >= 0
                                                && sy >= 0
                                                && sy < m_pixels.length
                                                && sx < m_pixels[sy].length
                                                && m_pixels[sy][sx] != Integer.MAX_VALUE) {
                                        matrix.incValueAt(m_pixels[y][x],
                                                        m_pixels[sy][sx]);
                                        nrPairs++;
                                }
                                ...
                                if (tx >= 0
                                                && ty >= 0
                                                && ty < m_pixels.length
                                                && tx < m_pixels[ty].length
                                                && m_pixels[ty][tx] != Integer.MAX_VALUE) {
                                        matrix.incValueAt(m_pixels[y][x],
                                                        m_pixels[ty][tx]);
                                        nrPairs++;
                                }
        ...
        public UnaryOperation<IterableInterval<T>, CooccurrenceMatrix> copy() {
                return new MakeCooccurrenceMatrix<T>(m_dimX, m_dimY,
                                m_distance, m_nrGrayLevels, m_orientation,
                                m_enabledFeatures);
        }
