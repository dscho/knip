...
import org.knime.knip.core.data.statistics.Histogram2D;
import org.knime.knip.core.exceptions.IntervalsNotCompatibleException;
...
public final class MakeHistogram2D<T extends RealType<T>>
        ...
        @Override
        public final Histogram2D createEmptyOutput(
                        final IterableInterval<T> op0,
                        final IterableInterval<T> op1) {
        ...
        public final Histogram2D compute(final IterableInterval<T> op0,
                ...
                if (!op0.iterationOrder().equals(op1.iterationOrder())) {
                        throw new IntervalsNotCompatibleException(
                                        "Intervals are not compatible in Histogram2D");
                }
                ...
                while (opc0.hasNext()) {
                        opc0.next();
                        opc1.next();
                        r.incByValue(opc0.get().getRealDouble(), opc1.get()
                                        .getRealDouble());
                }
                return r;
        }
        ...
        @Override
        public Histogram2D compute(IterableInterval<T> in1,
