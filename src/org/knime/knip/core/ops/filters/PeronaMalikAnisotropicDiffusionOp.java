...
public class PeronaMalikAnisotropicDiffusionOp<T extends RealType<T>>
    ...
    private DiffusionFunction m_fun;
    // number of threads
    private int threads;

    /**
     * 
     * Constructs a wrapping operation to execute the (elsewhere implemented)
     * Perona & Malik Anisotropic Diffusion scheme. See
     * {@link PeronaMalikAnisotropicDiffusion}.
     * 
     * @param deltat
     *            the integration constant for the numerical integration scheme.
     *            Typically less that 1.
     * @param n
	...
	// this is ugly and a hack but needed as the implementation of this
	// algorithms doesn't accept the input img

	CopyImgOperation<T> copyOp = new CopyImgOperation<T>();

	// build a new diffusion scheme
	PeronaMalikAnisotropicDiffusion<T> diff = new PeronaMalikAnisotropicDiffusion<T>(
		copyOp.compute(input, output), this.m_deltat, this.m_fun);
	...
	// do the process n times -> see {@link PeronaMalikAnisotropicDiffusion}
