...
public class WaveletFilter< T extends RealType< T >, K extends IterableInterval< T > & RandomAccessibleInterval< T >> implements UnaryOperation< K, K >
{
	...
	public K compute( final K src, final K res )
	{
		...
		if ( numDim > MAX_DIMS ) { throw new IllegalArgumentException( "Too many dimensions are selected." ); }
		if ( src.numDimensions() < MIN_DIMS ) { throw new IllegalArgumentException( "Two dimensions have to be selected." ); }
...

		{
			for ( int dim = 0; dim < numDim; ++dim )
			{
				if ( dimSrcCur[ dim ] )
				{
					if ( m_SelectedRowRoi == null || m_SelectedRowRoiOrigin.length != m_SelectedRowRoi.numDimensions() && m_BeginRoi == null || m_BeginRoiOrigin.length != m_BeginRoi.numDimensions() )
					{
					...
					m_SelectedRowTempRoiCur = m_SelectedRowRoi.getIterableIntervalOverROI( Views.extendValue( temp, obj ) ).cursor();
					...
					while ( m_BeginTempRoiCur.hasNext() )
					{
						...
						final FutureTask< WaveletDecompositionThread > task = new FutureTask< WaveletDecompositionThread >( wave );
...

		{
			...
			while ( m_tempCursor.hasNext() )
			{
				...
				if ( m_tempCursor.next().getRealDouble() < m_lambda_max && m_tempCursor.get().getRealDouble() > m_lambda_min )
				{
			...
			for ( int dim = numDim - 1; 0 <= dim; --dim )
			{
				if ( dimSrcCur[ dim ] )
				{
					...
					while ( m_BeginTempRoiCur.hasNext() )
					{
						...
						final FutureTask< WaveletCompositionThread > task = new FutureTask< WaveletCompositionThread >( wave );
	...
	class WaveletDecompositionThread implements Callable< WaveletDecompositionThread >
	{
	...
	public WaveletFilter( final ExecutorService executor, final double lambda_min, final double lambda_max, final double ignorePercent )
	{
		m_executor = executor;
