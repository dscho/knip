...
public class OtsuMultilevelThresholder<T extends RealType<T>, IN extends IterableInterval<T> & RandomAccessibleInterval<T>>
        ...
        @Override
        public ThresholdValueCollection compute(IN input,
                        ThresholdValueCollection thresholdValues)
                        throws RuntimeException {

                // Thresholds must be scaled
                T inVar = input.firstElement().createVariable();

                OpsHistogram histogram = new MakeHistogram<T>((int) Math.min(
                                m_numBins,
                                inVar.getMinValue() - inVar.getMaxValue()))
                                .compute(input);
                ...
                calculateThresholdValues(thresholdValues, 1, m_numBins
                                - m_numberOfLevels + 1, 0.0, 0,
                                tempThresholdList);

                // Threshold values must be scaled according to the number of
                // bins.
                thresholdValues.scale(m_numBins, inVar.getMinValue(),
                                inVar.getMaxValue());
        ...
        private void calculateThresholdValues(
                        ThresholdValueCollection thresholdValues, int start,
                        int end, double curValue, int curIndex, int[] tList) {
                ...
                for (int i = start; i < end; i++) {
                        ...
                        if (curIndex == m_numberOfLevels - 2) {
                                ...
                                calculateThresholdValues(thresholdValues,
                                                i + 1, end + 1, curValue + h1,
                                                curIndex + 1, tList);
                        }
                }
        }
        ...
        public UnaryOutputOperation<IN, ThresholdValueCollection> copy() {
                return new OtsuMultilevelThresholder<T, IN>(m_numberOfLevels,
                                m_numBins);
        }
